...
def parse_pt_date(date_string):
    ...
    if date_info.count("de") == 2 or len(date_info) == 3:
        ...
        total_offset_seconds = offset_signal * (
            offset_hours * 3600 + offset_minutes * 60
        )
        offset_in_days = total_offset_seconds / (3600.0 * 24)

        month = MONTHS[month_pt]
        datetime_iso = "{}-{:02d}-{:02d}T{}".format(
            year, month, int(day), hour_minute_second
        )
...
if __name__ == "__main__":
    assert parse_pt_date("Seg, 21 Out 2013 21:14:36 -0200") == datetime.datetime(
        2013, 10, 21, 23, 14, 36
    )

    assert parse_pt_date("terça-feira, 9 de outubro de 2012") == datetime.datetime(
        2012, 10, 9, 0, 0, 0
    )

    assert parse_pt_date("Ter, 31 Jul 2012 16:54:00 GMT") == datetime.datetime(
        2012, 7, 31, 16, 54, 0
    )

    assert parse_pt_date("23 Abr 2008") == datetime.datetime(2008, 4, 23, 0, 0, 0)

    assert parse_pt_date("23 de Dezembro de 2013") == datetime.datetime(
        2013, 12, 23, 0, 0, 0
    )

    print(parse_pt_date("Ter, 31 Jul 2012 16:54:00 GMT"))
    print(parse_pt_date("terça-feira, 9 de outubro de 2012"))
